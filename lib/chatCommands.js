...
class ChatCommands {
  ...
  async kamp (channel, argument) {
    ...
    const ourStreams = matches.data.data.filter(x => x.videos.some(v => v.source === 'twitch' && v.remote_id === channel))
  ...
  async findNewResults (channel) {
    ...
    const queryParams = {
      ...
      from_data: '2023-04-10',
      //from_date: new Date().toISOString().slice(0, 10),
      limit: 10
    }

    if (this.data[channel].divisjon.id != undefined) {
